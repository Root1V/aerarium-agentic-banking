...
func TestUnaAutorizacionRetieneElDineroSinCobrarlo(t *testing.T) {
	f := setup(t, 500_00)
	purchase := cardsim.NewPurchase(f.cardID, 120_00, "USD", "Cafetería")
	...
	if got := f.balance(t, f.accountID); got != 380_00 {
		t.Errorf("saldo disponible = %d, se esperaba 38000", got)
	}
	if got := f.balance(t, f.accounts.AuthHoldID); got != 120_00 {
...
func TestSinFondosSeRechazaConMotivo(t *testing.T) {
	f := setup(t, 50_00)
	purchase := cardsim.NewPurchase(f.cardID, 200_00, "USD", "Tienda")
	...
	if got := f.balance(t, f.accountID); got != 50_00 {
...
func TestUnaTarjetaCongeladaRechaza(t *testing.T) {
	...
	decision, err := f.svc.Authorize(f.ctx, cardsim.NewPurchase(f.cardID, 10_00, "USD", "Tienda").Authorization())
	if err != nil {
		t.Fatalf("autorizar: %v", err)
	}
	...
	if got := f.balance(t, f.accountID); got != 500_00 {
...
func TestUnaAutorizacionRepetidaNoRetieneDosVeces(t *testing.T) {
	f := setup(t, 500_00)
	request := cardsim.NewPurchase(f.cardID, 90_00, "USD", "Tienda").Authorization()
	...
	if got := f.balance(t, f.accounts.AuthHoldID); got != 90_00 {
...
func TestAutorizacionesConcurrentesDeLaMismaCompra(t *testing.T) {
	f := setup(t, 500_00)
	request := cardsim.NewPurchase(f.cardID, 75_00, "USD", "Tienda").Authorization()
	...
	if got := f.balance(t, f.accounts.AuthHoldID); got != 75_00 {
...
func TestElCobroPorElMontoAutorizadoCierraLaRetencion(t *testing.T) {
	...
	result, err := f.svc.Clear(f.ctx, purchase.Clearing(120_00))
	if err != nil {
		t.Fatalf("cobrar: %v", err)
	}

	if result.OverageMinor != 0 || result.ReturnedMinor != 0 {
...
func TestSiElComercioCobraMenosSeDevuelveLaDiferencia(t *testing.T) {
	f := setup(t, 500_00)
	purchase := cardsim.NewPurchase(f.cardID, 100_00, "USD", "Estación de servicio")

	if _, err := f.svc.Authorize(f.ctx, purchase.Authorization()); err != nil {
		t.Fatalf("autorizar: %v", err)
	}
	result, err := f.svc.Clear(f.ctx, purchase.Clearing(60_00))
	if err != nil {
		t.Fatalf("cobrar: %v", err)
	}

	if result.ReturnedMinor != 40_00 {
		t.Errorf("devuelto = %d, se esperaba 4000", result.ReturnedMinor)
	}
	if got := f.balance(t, f.accountID); got != 440_00 {
...
func TestSiElComercioCobraMasSeCargaLaDiferenciaAlCliente(t *testing.T) {
	f := setup(t, 500_00)
	purchase := cardsim.NewPurchase(f.cardID, 100_00, "USD", "Restaurante")

	if _, err := f.svc.Authorize(f.ctx, purchase.Authorization()); err != nil {
		t.Fatalf("autorizar: %v", err)
	}
	result, err := f.svc.Clear(f.ctx, purchase.Clearing(118_00))
	if err != nil {
		t.Fatalf("cobrar: %v", err)
	}

	if result.OverageMinor != 0 {
		t.Errorf("el cliente tenía saldo: el banco no debía adelantar nada, %+v", result)
	}
	if got := f.balance(t, f.accountID); got != 382_00 {
		t.Errorf("saldo = %d, se esperaba 38200", got)
	}
	if got := f.balance(t, f.accounts.SettlementID); got != -118_00 {
...
func TestUnExcedenteSinSaldoLoAdelantaElBancoYQuedaRegistrado(t *testing.T) {
	...
	result, err := f.svc.Clear(f.ctx, purchase.Clearing(115_00))
	if err != nil {
		t.Fatalf("el cobro NO puede fallar: el dinero ya se gastó (%v)", err)
	}

	if result.OverageMinor != 15_00 {
		t.Errorf("excedente adelantado = %d, se esperaba 1500", result.OverageMinor)
	}
	if got := f.balance(t, f.accountID); got != 0 {
		t.Errorf("saldo del cliente = %d: no debe quedar en negativo", got)
	}
	if got := f.balance(t, f.accounts.OverageID); got != 15_00 {
		t.Errorf("cartera por cobrar = %d: el adelanto debe quedar visible", got)
	}
	if got := f.balance(t, f.accounts.SettlementID); got != -115_00 {
	...
	if stored.OverageMinor != 15_00 {
		t.Errorf("el adelanto debe quedar registrado, se obtuvo %d", stored.OverageMinor)
	}
}

func TestUnCobroRepetidoNoCobraDosVeces(t *testing.T) {
	f := setup(t, 500_00)
	purchase := cardsim.NewPurchase(f.cardID, 80_00, "USD", "Tienda")

	if _, err := f.svc.Authorize(f.ctx, purchase.Authorization()); err != nil {
		t.Fatalf("autorizar: %v", err)
	}
	clearing := purchase.Clearing(80_00)
	...
	if got := f.balance(t, f.accountID); got != 420_00 {
...
func TestUnaReversaLiberaElDineroRetenido(t *testing.T) {
	f := setup(t, 500_00)
	purchase := cardsim.NewPurchase(f.cardID, 130_00, "USD", "Hotel")
...
func TestNoSePuedeCobrarUnaCompraYaReversada(t *testing.T) {
	f := setup(t, 500_00)
	purchase := cardsim.NewPurchase(f.cardID, 50_00, "USD", "Tienda")
	...
	if _, err := f.svc.Clear(f.ctx, purchase.Clearing(50_00)); err == nil {
...
func TestSeListanLasRetencionesVivasParaLiberarlas(t *testing.T) {
	f := setup(t, 500_00)
	purchase := cardsim.NewPurchase(f.cardID, 40_00, "USD", "Hotel")
