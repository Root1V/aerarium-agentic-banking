...
impl ProductRepository {
    ...
    pub async fn create(&self, product: &NewProduct) -> Result<Product, PostingError> {
        ...
        if product.max_balance_minor.is_some_and(|cap| cap <= 0) {
            return Err(PostingError::Invalid("max_balance_minor must be positive".into()));
        }
        if product.max_transaction_minor.is_some_and(|cap| cap <= 0) {
            return Err(PostingError::Invalid("max_transaction_minor must be positive".into()));
        }

        let r = sqlx::query!(
            r#"
            INSERT INTO products (code, name, kind, currency, allows_overdraft,
                                  max_balance_minor, max_transaction_minor)
            VALUES ($1, $2, $3, $4, $5, $6, $7)
            RETURNING id, code, name, kind as "kind: ProductKind", currency,
                      allows_overdraft, max_balance_minor, max_transaction_minor, active
            "#,
            product.code,
            product.name,
            ...
            product.max_balance_minor,
            product.max_transaction_minor,
        )
        .fetch_one(&self.pool)
        .await
        ...
        Ok(Product {
            ...
            max_balance_minor: r.max_balance_minor,
            max_transaction_minor: r.max_transaction_minor,
            active: r.active,
        })
    }
