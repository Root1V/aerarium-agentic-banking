...
#: Límite mínimo con el que tiene sentido abrir una línea.
MIN_LIMIT_MINOR = 20_000
#: Techo del primer límite, por alto que sea el puntaje. Nadie estrena una línea
#: grande: primero hay que ver cómo paga.
MAX_INITIAL_LIMIT_MINOR = 500_000
...
def decide(
    ...
    *,
    ...
    observed_enough = (
        months_of_history >= MIN_MONTHS_FOR_DECLINE and average_monthly_inflow_minor > 0
    )
    ...
    limit = _limit_for(score_result.value, average_monthly_inflow_minor, months_of_history)

    if limit < MIN_LIMIT_MINOR:
...
def _limit_for(score: int, average_monthly_inflow_minor: int, months_of_history: int) -> int:
    ...
    un puntaje alto.
    """
    by_income = int(average_monthly_inflow_minor * MAX_INCOME_SHARE)

    # El puntaje modula dentro de lo que el ingreso permite.
    score_factor = min(1.0, (score - APPROVAL_THRESHOLD) / 300 + 0.4)
    by_score = int(MAX_INITIAL_LIMIT_MINOR * score_factor)

    # Rampa por antigüedad: seis meses de relación para acceder al máximo.
    tenure_factor = min(1.0, months_of_history / 6)
    by_tenure = int(MAX_INITIAL_LIMIT_MINOR * tenure_factor)

    limit = min(by_income, by_score, by_tenure, MAX_INITIAL_LIMIT_MINOR)

    # Se redondea hacia abajo a la unidad para no ofrecer límites con centavos.
